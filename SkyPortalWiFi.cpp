...
int SkyPortalWiFi::Connect(char *pszPort)
{
	...
	timestamp = asctime(localtime(&ltime));
	timestamp[strlen(timestamp) - 1] = 0;
	fprintf(Logfile, "[%s] SkyPortalWiFi::Connect Called %s\n", timestamp, pszPort);
    fflush(Logfile);
#endif
...
int SkyPortalWiFi::SkyPortalWiFiSendCommand(const unsigned char *pszCmd, unsigned char *pszResult, int nResultMaxLen)
{
    ...
    fprintf(Logfile, "[%s] CSkyPortalWiFiController::SkyPortalWiFiCommand Sending %s\n", timestamp, cHexMessage);
    fflush(Logfile);
#endif
    ...
    if(pszResult) {
        ...
        while(szResp[DST_DEV]!=PC) {
            ...
            fprintf(Logfile, "[%s] CSkyPortalWiFiController::SkyPortalWiFiCommand response \"%s\"\n", timestamp, cHexMessage);
            fflush(Logfile);
#endif
            if(nErr)
                return nErr;
        ...
        fprintf(Logfile, "[%s] CSkyPortalWiFiController::SkyPortalWiFiCommand response copied to pszResult : \"%s\"\n", timestamp, cHexMessage);
        fflush(Logfile);
#endif
    }
    return nErr;
...
int SkyPortalWiFi::getRaAndDec(double &dRa, double &dDec)
{
    ...
    nErr = getPosition(nAltSteps, nAzSteps);
    if(nErr)
        return nErr;
    ...
    // now deal with A;t/Az mount as we need to adjust the tracking rate all the time
    if(m_bIsTracking && !m_bMountIsGEM) {
        // adjust tracking rate every 30 seconds at most
        if(m_trakingTimer.GetElapsedSeconds() <= 30) {
            return nErr;
        }
        ...
        // difference between we were and now
        // traking rates are in steps per minute and we check every 30 seconds or so,
        // so we multiply the differece by 2
        nAltRate = (nNewAltSteps - m_nPrevAltSteps + nAltError)  * 2;
        nAzRate = (nNewAzSteps - m_nPrevAzSteps + nAzError) * 2;
        ...
        fprintf(Logfile, "[%s] [SkyPortalWiFi::getRaAndDec] Scpoe Az steps  : %d\n", timestamp, nAzSteps);
        ...
        fprintf(Logfile, "[%s] [SkyPortalWiFi::getRaAndDec] Target          : %3.2f\n", timestamp, dNewAz);
        ...
        nErr = setTrackingRatesSteps(nAltSteps, nAzRate, 3);
...
int SkyPortalWiFi::getPosition(int &nAltSteps, int &nAzSteps)
{
    int nErr = SKYPORTAL_OK;
    unsigned char szCmd[SERIAL_BUFFER_SIZE];
    unsigned char szResp[SERIAL_BUFFER_SIZE];
...
int SkyPortalWiFi::setPosition(int nAltSteps, int nAzSteps)
{
...
int SkyPortalWiFi::syncTo(double dRa, double dDec)
{
    ...
    degToSteps(dAzPos, nAltPos);
    degToSteps(dAltPos, nAzPos);
    ...
    fflush(Logfile);
#endif
    nErr = setPosition(nAltPos, nAzPos);
    if(!nErr)
        m_bIsSynced = true;
    return nErr;
}
...
int SkyPortalWiFi::setTrackingRates(bool bTrackingOn, bool bIgnoreRates, double dTrackRaArcSecPerMin, double dTrackDecArcSecPerMin)
{
    ...
    if(!bTrackingOn) { // stop tracking
        ...
        nErr |= setTrackingRatesSteps(0, 0xFFFF, 2); // set Ra to sidereal
        if(nErr)
            return nErr;
        m_bSiderealOn = true;
    }
        ...
        if(m_bMountIsGEM) {
            ...
            nErr = setTrackingRatesSteps(nDecSteps, nRaSteps, 3);
            if(nErr)
                return nErr;
        }
        else {
            ...
            // nErr = setTrackingRatesSteps(nAltRate, nAzRate, 3);
        }
    }
    return nErr;
}
...
int SkyPortalWiFi::isSlewToComplete(bool &bComplete)
{
    ...
    nErr = getPosition(m_nPrevAltSteps, m_nPrevAzSteps);
    if(nErr)
        return nErr;
...
int SkyPortalWiFi::getAtPark(bool &bParked)
{
    ...
    getPosition(nAltSteps, nAzSteps);
...
int SkyPortalWiFi::setTrackingRatesSteps(int nAltRate, int nAzRate, int nDataLen)
{
    int nErr = SKYPORTAL_OK;
    unsigned char szCmd[SERIAL_BUFFER_SIZE];
    unsigned char szResp[SERIAL_BUFFER_SIZE];
